...
int second(int number,int power){
	int res=1;
...
void main(void)
  {
	...
	index=3;
	word buzzerTimer=0;
	SetLedBar(second(2,2)-1);
	while(1){
		if(GetKeyboard()=='C' && run==0){
			Buzzer=1;
			buzzerTimer=0;
			printf("7 - Stop\n");
			printf("Rotace vlny\n");
		...
		if(GetKeyboard()=='7' && run==1){
			Buzzer=1;
			buzzerTimer=0;
			if(index<11)
				printf("Kombinace 0x%x\n",second(2,index)-1);
			else
...
void timer0() __interrupt 1
  {
	...
	if(++TIMER>=300){
		if(run==1){
			TIMER=0;
			if(index <11)
				SetLedBar(second(2,index)-1);
			else
				SetLedBar(1023-(second(2,index-10)-1));
			if(index>=20)
				index=2;
			else
				index++;
